...
function updateSelectedPlanet(mouse) {
	...
	for (var o in system.orbits) {
		for (var i in system.orbits[o].planets) {
			let planet = system.orbits[o].planets[i]
			...
			if (planet != sendShipsFrom) {
				...
				if (updateLines == 0) {
					...
					// TODO make the planets move forward to the spot where the ships will collide
					if (!collides) {
						for (var k in system.orbits) {
							for (var j in system.orbits[k].planets) {
								// current planet of interest
								let current = system.orbits[k].planets[j]
...
function cancelSendShips() {
	for (var i in system.orbits) {
		for (var j in system.orbits[i].planets) {
			let planet = system.orbits[i].planets[j]
			planet.outline.visible = false
