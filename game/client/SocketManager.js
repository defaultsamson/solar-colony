  ...
  parse(type, pack) {
    switch (type) {
      ...
      case Pack.CREATE_ORBIT:
        let orbit = new Orbit(pack.x, pack.y, pack.radius)
        orbit.id = pack.id
        game.system.addOrbit(orbit)
        break

      case Pack.CREATE_PLANET:
        let planet = new Planet(resources.planet1.texture, pack.scale, pack.rotationConstant, pack.startAngle, pack.opm)
        planet.id = pack.id
        let orbit = game.system.getOrbit(pack.orbit)
        orbit.addPlanet(planet)
        break
        */

      case Pack.CREATE_SPAWN: {
        let planet = game.system.getPlanetByID(pack.planet)
      ...
      case Pack.SET_PLANET_TEAM: {
        ...
        let planet = game.system.getPlanetByID(pack.planet)
        let team = game.getTeam(pack.team)
        planet.setTeam(team)
      }
        break
